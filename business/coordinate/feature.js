...
export function createCoordinateFeature({ repository, excerpts = null, root = null, capture = captureMesText, ports = null, host = {} } = {}) {
    ...
    const bindExcerpts = target => {
        const clickOff = ui.bind(target, 'click', async event => {
            ...
            if (el.matches('.sp-excerpt-del')) {
                if ((await host.confirm?.({ title: '删除摘抄', body: '删除这条摘抄？快照还在。', confirmText: '删除', cancelText: '取消' })) === false) return;
    ...
    return {
        ...
        open(route) { ui.open(route); controller.beginView(route || ui.route()); return Promise.resolve(repository.hydrateSearchText?.()).catch(() => 0).then(() => renderCurrent(() => renderer.render())); },
        openAtChat(chatId) { ui.setRoute({ level: 'items', chatId }); controller.beginView('items', chatId); return Promise.resolve(repository.hydrateSearchText?.()).catch(() => 0).then(() => renderCurrent(() => renderer.render())); },
