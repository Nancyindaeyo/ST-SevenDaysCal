...
export function createCoordinateRenderer({ repository, excerptRepo = null, setBody, getState, setState, getTheme = () => 'day', escapeHtml = esc, formatTimestamp = value => new Date(value || 0).toLocaleString(), svg = () => '', documentRef = globalThis.document, queryRoot = null } = {}) {
    ...
    const itemCard = (item, map, { showChat = false } = {}) => {
        ...
        const floor = item.kind === 'theater' ? '棱' : `#${item.floorIndex ?? '?'}`;
        return `<article class="sp-anchor-item-card" data-id="${esc(item.id)}" data-search="${esc(itemHay(item, map))}"><button type="button" class="sp-anchor-item-open"><span class="sp-anchor-item-tags">${tagChips(item.tags, map)}</span><span class="sp-anchor-item-main"><span class="sp-anchor-item-floor">${esc(floor)}</span><span class="sp-anchor-item-preview">${escapeHtml(item.textPreview || '(无正文预览)')}</span><span class="sp-anchor-item-ts">${formatTimestamp(item.ts)}</span></span>${chat}</button><input type="text" class="sp-anchor-item-note-input" data-id="${esc(item.id)}" maxlength="80" placeholder="写一句备注…" value="${esc(item.note || '')}" autocomplete="off" spellcheck="false"></article>`;
    };
    ...
    async function items(chatId) {
        ...
        setBody(`<div class="sp-anchor-head"><button type="button" class="sp-anchor-back" data-to="chars">‹</button><span class="sp-anchor-head-title">${escapeHtml(bucket.chatName || bucket.charName || '收藏')}</span><span class="sp-anchor-head-count">${list.length} 条</span></div><div class="sp-anchor-scroll">${searchBox('snaps', s.snapSearch, SNAP_SEARCH_HINT)}${filterBar(tags, s.filter)}<div class="sp-anchor-item-list" data-filter-list>${list.map(item => itemCard(item, map)).join('') || '<div class="sp-anchor-filter-empty">没有含此分组的收藏</div>'}</div>${searchEmpty('没有匹配的快照')}</div>`);
    }
    async function full(itemId, revision = null, theme = null) {
        ...
        if (host) {
            ...
            if (shadow) {
                ...
                shadow.innerHTML = `<style>:host{all:initial;display:block;color:${fg};-webkit-user-select:text;user-select:text}.sp-anchor-snap{display:block;color:${fg};background:${bg};padding:16px 18px !important;margin:0 !important;border:none !important;border-radius:10px;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,"PingFang SC","Microsoft YaHei",sans-serif;font-size:12px;line-height:1.6;word-break:break-word;user-select:text !important;-webkit-user-select:text !important;-webkit-touch-callout:default;touch-action:pan-y;cursor:text}.sp-anchor-snap ::selection{background:${mark}}.sp-anchor-snap img{max-width:100%;height:auto}.sp-anchor-snap a{color:${link}}.sp-anchor-snap q:before,.sp-anchor-snap q:after{content:''}.sp-anchor-pick{-webkit-user-select:text;user-select:text}.sp-anchor-pick-on{background:${mark};border-radius:4px;box-decoration-break:clone;-webkit-box-decoration-break:clone}@media (pointer:coarse),(hover:none),(max-width:640px){.sp-anchor-snap{-webkit-user-select:none;user-select:none;-webkit-touch-callout:none}.sp-anchor-pick{cursor:pointer}}</style><div class="mes_text sp-anchor-snap">${item.html || ''}</div>`;
                wrapPickableText(shadow.querySelector('.sp-anchor-snap'), documentRef);
            }
        }
        return item;
    }
    async function excerpts() {
        ...
        const empty = list.length
            ? ''
            : '<div class="sp-excerpt-empty"><p>快照是整楼副本，摘抄是你划下的句子和点评，两边互不覆盖。</p><p>打开一条快照：手机点句子勾选，电脑拖选，再点「摘抄选中」。发给间会做成引用条，输入框留给你接着写，不会代你发送。</p></div>';
