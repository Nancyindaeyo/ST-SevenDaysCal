...
export function createCoordinateRenderer({ repository, excerptRepo = null, setBody, getState, setState, getTheme = () => 'day', escapeHtml = esc, formatTimestamp = value => new Date(value || 0).toLocaleString(), svg = () => '', documentRef = globalThis.document, queryRoot = null } = {}) {
    ...
    const searchBox = (shelf, value, placeholder) => `<div class="sp-excerpt-search-wrap"><i class="fa-solid fa-magnifying-glass"></i><input type="text" class="sp-anchor-search sp-input" data-shelf="${shelf}" placeholder="${esc(placeholder)}" value="${esc(value || '')}" autocomplete="off" spellcheck="false"></div>`;
    ...
    async function chars() {
        ...
        setBody(`${await snapHead(s, '<button type="button" class="sp-icon-btn sp-anchor-tagmgr-btn" title="管理分组" aria-label="管理分组">分组</button>')}<div class="sp-anchor-scroll">${searchBox('snaps', s.snapSearch, '搜角色、预览、分组名…')}<div class="sp-anchor-char-list" data-filter-list>${cards || '<div class="sp-anchor-filter-empty">没有含此分组的收藏</div>'}</div>${searchEmpty('没有匹配的角色')}</div>`);
    }
    ...
    async function group() {
        ...
        setBody(`<div class="sp-anchor-head"><button type="button" class="sp-anchor-back" data-browse="tag">‹</button><span class="sp-anchor-head-title">${escapeHtml(packed.name)}</span><span class="sp-anchor-head-count">${list.length} 条</span></div><div class="sp-anchor-scroll">${searchBox('snaps', s.snapSearch, '搜预览、角色…')}<div class="sp-anchor-item-list" data-filter-list>${list.map(item => itemCard(item, map, { showChat: true })).join('') || '<div class="sp-anchor-filter-empty">这一组还是空的</div>'}</div>${searchEmpty('没有匹配的快照')}</div>`);
    }
    async function tags() {
    ...
    async function items(chatId) {
        ...
        if (s.charName && (s.chatId == null || s.chatId === '')) {
            ...
            setBody(`<div class="sp-anchor-head"><button type="button" class="sp-anchor-back" data-to="chars">‹</button><span class="sp-anchor-head-title">${escapeHtml(s.charName)}</span><span class="sp-anchor-head-count">${list.length} 条</span></div><div class="sp-anchor-scroll">${searchBox('snaps', s.snapSearch, '搜预览、分组…')}${filterBar(tags, s.filter)}<div class="sp-anchor-item-list" data-filter-list>${list.map(item => itemCard(item, map, { showChat: true })).join('') || '<div class="sp-anchor-filter-empty">没有含此分组的收藏</div>'}</div>${searchEmpty('没有匹配的快照')}</div>`);
            return;
        }
