...
export function createCoordinateRenderer({ repository, excerptRepo = null, setBody, getState, setState, getTheme = () => 'day', escapeHtml = esc, formatTimestamp = value => new Date(value || 0).toLocaleString(), svg = () => '', documentRef = globalThis.document, queryRoot = null } = {}) {
    ...
    const searchBox = (shelf, value, placeholder) => `<label class="sp-excerpt-search-wrap"><i class="fa-solid fa-magnifying-glass"></i><input type="text" class="sp-anchor-search sp-input" data-shelf="${shelf}" placeholder="${esc(placeholder)}" value="${esc(value || '')}" autocomplete="off" spellcheck="false"></label>`;
    ...
    const itemCard = (item, map) => {
        const hay = hayOf([item.charName, item.chatName, item.textPreview, item.floorIndex, ...tagNamesOf(item.tags, map)]);
        return `<button type="button" class="sp-anchor-item-card" data-id="${esc(item.id)}" data-search="${esc(hay)}"><span class="sp-anchor-item-tags">${tagChips(item.tags, map)}</span><span class="sp-anchor-item-main"><span class="sp-anchor-item-floor">#${item.floorIndex ?? '?'}</span><span class="sp-anchor-item-preview">${escapeHtml(item.textPreview || '(无正文预览)')}</span><span class="sp-anchor-item-ts">${formatTimestamp(item.ts)}</span></span></button>`;
    };
    ...
    async function chars() {
        ...
        for (const bucket of all) { const items = visible(bucket.items || [], s.filter); if (!items.length) continue; const key = bucket.charName || '(未知角色)'; const group = groups.get(key) || { name: key, chatCount: 0, count: 0, latestTs: 0, hay: [] }; group.chatCount++; group.count += items.length; group.latestTs = Math.max(group.latestTs, ...items.map(item => item.ts || 0)); group.hay.push(key, ...items.map(item => item.textPreview || ''), ...items.flatMap(item => tagNamesOf(item.tags, map))); groups.set(key, group); }
        const list = [...groups.values()].sort((a, b) => b.latestTs - a.latestTs);
        const cards = list.map(group => `<button type="button" class="sp-anchor-char-card" data-char="${esc(group.name)}" data-search="${esc(hayOf(group.hay))}"><span class="sp-anchor-chat-icon">${svg('sp-anchor-chat-svg')}</span><span class="sp-anchor-chat-main"><span class="sp-anchor-chat-name">${escapeHtml(group.name)}</span><span class="sp-anchor-chat-sub">${group.chatCount} 个聊天</span></span><span class="sp-anchor-chat-meta"><span class="sp-anchor-chat-count">${group.count}</span><span class="sp-anchor-chat-ts">${formatTimestamp(group.latestTs)}</span></span></button>`).join('');
        setBody(`${await snapHead(s, '<button type="button" class="sp-icon-btn sp-anchor-tagmgr-btn" title="管理分组" aria-label="管理分组">分组</button>')}<div class="sp-anchor-scroll">${searchBox('snaps', s.snapSearch, '搜角色、预览、分组名…')}${filterBar(tags, s.filter)}<div class="sp-anchor-char-list" data-filter-list>${cards || '<div class="sp-anchor-filter-empty">没有含此分组的收藏</div>'}</div>${searchEmpty('没有匹配的角色')}</div>`);
    }
    async function tagHome() {
        ...
        const groupCards = groups.map(group => card(group.tag.id, group.tag.name, group.items.length, Math.max(0, ...group.items.map(item => item.ts || 0)), hayOf([group.tag.name, ...group.items.map(item => item.textPreview), ...group.items.map(item => item.charName)]), group.tag.color || 'slate')).join('');
        const noneCard = untagged.length ? card('__none__', '未分组', untagged.length, Math.max(0, ...untagged.map(item => item.ts || 0)), hayOf(['未分组', ...untagged.map(item => item.textPreview), ...untagged.map(item => item.charName)])) : '';
    ...
    async function group() {
        ...
        setBody(`<div class="sp-anchor-head"><button type="button" class="sp-anchor-back" data-browse="tag">‹</button><span class="sp-anchor-head-title">${escapeHtml(packed.name)}</span><span class="sp-anchor-head-count">${list.length} 条</span></div><div class="sp-anchor-scroll">${searchBox('snaps', s.snapSearch, '搜预览、角色…')}<div class="sp-anchor-item-list" data-filter-list>${list.map(item => itemCard(item, map)).join('') || '<div class="sp-anchor-filter-empty">这一组还是空的</div>'}</div>${searchEmpty('没有匹配的快照')}</div>`);
    }
    async function tags() {
    ...
    async function items(chatId) {
        ...
        setBody(`<div class="sp-anchor-head"><button type="button" class="sp-anchor-back" data-to="chats" data-char="${esc(bucket.charName)}">‹</button><span class="sp-anchor-head-title">${escapeHtml(bucket.chatName || bucket.charName || '收藏')}</span><span class="sp-anchor-head-count">${list.length} 条</span></div><div class="sp-anchor-scroll">${searchBox('snaps', s.snapSearch, '搜预览、分组…')}${filterBar(tags, s.filter)}<div class="sp-anchor-item-list" data-filter-list>${list.map(item => itemCard(item, map)).join('') || '<div class="sp-anchor-filter-empty">没有含此分组的收藏</div>'}</div>${searchEmpty('没有匹配的快照')}</div>`);
    }
    async function full(itemId, revision = null, theme = null) {
        ...
        setState?.({ level: 'full', itemId, shelf: 'snaps' }); const s = current(); const edit = s.fullTagEdit ? `<div class="sp-anchor-full-tagedit"><div class="sp-anchor-ftag-chips">${tags.map(tag => `<button type="button" class="sp-anchor-ftag-chip${item.tags?.includes(tag.id) ? ' sp-tp-chip-on' : ''}" data-id="${esc(tag.id)}"><span class="sp-anchor-tagchip" data-color="${esc(tag.color || 'slate')}">${esc(tag.name)}</span></button>`).join('')}</div><div class="sp-anchor-ftag-new"><input class="sp-anchor-ftag-name sp-input" placeholder="新建分组名…"></div><div class="sp-anchor-ftag-foot"><button type="button" class="sp-anchor-ftag-add sp-mini-btn">新增</button><button type="button" class="sp-anchor-ftag-done sp-mini-btn">完成</button></div></div>` : `<div class="sp-anchor-full-tags">${tagChips(item.tags, map)}<button type="button" class="sp-anchor-tag-edit sp-mini-btn">编辑分组</button></div>`; setBody(`<div class="sp-anchor-head"><button type="button" class="sp-anchor-back" data-to="items" data-chatid="${esc(item.chatId)}">‹</button><span class="sp-anchor-head-title">${escapeHtml(item.charName || '')}<span class="sp-anchor-head-floor"> · #${item.floorIndex ?? '?'}</span></span><span class="sp-anchor-head-actions"><button type="button" class="sp-icon-btn sp-anchor-clip" title="把划选的句子做成摘抄">摘抄选中</button><button type="button" class="sp-icon-btn sp-anchor-fullscreen" title="全屏浏览">⛶</button><button type="button" class="sp-icon-btn sp-anchor-del" title="删除此收藏"><i class="fa-solid fa-trash"></i></button></span></div><div class="sp-anchor-scroll">${edit}${composerHtml(s.composer)}<div class="sp-anchor-full-host" id="sp-anchor-full-host"></div><p class="sp-anchor-clip-hint">手机长按划一段，电脑直接拖选，再点右上角「摘抄选中」。摘抄单独保存，删快照也不会带走它。</p><div class="sp-anchor-full-ts">收藏于 ${formatTimestamp(item.ts)}</div></div><div class="sp-anchor-fs-resize" title="拖拽调整大小"></div>`);
    ...
    async function excerpts() {
        ...
        const cards = list.map(item => {
            ...
            const actions = editing
                ...
                : `<div class="sp-excerpt-actions"><button type="button" class="sp-excerpt-locate sp-mini-btn" data-id="${esc(item.id)}" title="打开对应快照，不切换酒馆聊天档">看快照</button><button type="button" class="sp-excerpt-send-space sp-mini-btn" data-id="${esc(item.id)}" title="填进间的输入框，不代发">发给间</button><button type="button" class="sp-excerpt-edit sp-mini-btn" data-id="${esc(item.id)}">改点评</button><button type="button" class="sp-excerpt-del sp-mini-btn" data-id="${esc(item.id)}">删除</button></div>`;
        ...
        const empty = list.length
            ? ''
            : '<div class="sp-excerpt-empty"><p>快照是整楼副本，摘抄是你划下的句子和点评，两边互不覆盖。</p><p>打开一条快照，划选文字，点「摘抄选中」。发给间会把摘抄填进输入框，不会代你发送。</p></div>';
    ...
    async function render() {
        ...
        if (s.level === 'items' && s.chatId != null) return items(s.chatId);
