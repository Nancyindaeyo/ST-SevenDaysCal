export function createAdvanceQueue(env = {}) {
    ...
    async function safelyExecute(options) {
        try {
            ...
            try { env.onError?.(error, options); } catch {}
    ...
    function run(options = {}) {
        ...
        if (options.trigger !== 'date') return Promise.resolve({ status: 'skipped', reason: 'busy' });
        if (pendingDate) pendingDate.resolve({ status: 'skipped', reason: 'superseded', supersededBy: options.messageId ?? null });
        return new Promise(resolve => { pendingDate = { options, resolve }; });
    }
