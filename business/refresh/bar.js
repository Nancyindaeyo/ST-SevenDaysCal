...
export function refreshBarHtml({ selected = ['point', 'lines'], reason = '', feedback = '', outlineMode = 'current' } = {}) {
    const on = name => selected.includes(name) ? ' checked' : '';
    const mode = name => outlineMode === name ? ' checked' : '';
    return `<div class="sp-refresh-bar" id="sp-refresh-bar">
        ...
        <div class="sp-refresh-bar-row" id="sp-refresh-outline-mode">
            <span class="sp-refresh-bar-label">面怎么重做</span>
            <label class="sp-mode-opt"><input type="radio" name="sp-refresh-outline-mode" value="current"${mode('current')}><span>当前节点</span></label>
            <label class="sp-mode-opt"><input type="radio" name="sp-refresh-outline-mode" value="continue"${mode('continue')}><span>续写</span></label>
            <label class="sp-mode-opt"><input type="radio" name="sp-refresh-outline-mode" value="all"${mode('all')}><span>全部替换</span></label>
        </div>
...
export function readRefreshBar($root) {
    if (!$root?.length) return { selected: [], reason: '', feedback: '', outlineMode: 'current' };
    const selected = [];
    $root.find('.sp-refresh-mod:checked').each(function () { selected.push(String(this.getAttribute('data-mod') || '')); });
    const outlineMode = String($root.find('input[name="sp-refresh-outline-mode"]:checked').val() || 'current');
    return {
        ...
        outlineMode: outlineMode === 'all' || outlineMode === 'continue' ? outlineMode : 'current',
    };
}
