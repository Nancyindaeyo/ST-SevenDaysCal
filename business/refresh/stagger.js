export function createStaggerGate() {
  ...
  return {
    ...
    hydrate({
      pendingAdvance: nextAdvance = false,
      pendingDashed: nextDashed = false,
    } = {}) {
    ...
    get pendingDashed() {
      return pendingDashed;
    },
  };
}

export function shouldDeferAdvance({
  reconcileRan,
  wouldAdvance,
  pending = false,
} = {}) {
  if (reconcileRan && (wouldAdvance || pending))
    return { advance: false, pending: true };
