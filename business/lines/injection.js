...
export function createLinesInjectionController({ context, settings, enabled, readRaw, adultMode = () => 'off', lineDirection = () => 'natural', promptTypes = {}, promptRoles = {}, clean = value => value } = {}) {
    const refresh = () => {
        ...
        const text = buildLinesInjection(lines.map(line => ({ ...line, next: line.next ? prefixNext(line.next, line.stall) : line.next })), { adultMode: adultMode(), lineDirection: lineDirection() }).replace(/\r/g, '').replace(/  (.*)/g, (_, value) => `  ${clean(value)}`);
