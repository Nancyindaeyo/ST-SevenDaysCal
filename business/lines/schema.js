...
const lineAnchorKind = value => {
    ...
    return /^(?:Ticket|Cue|Adult|Dormant|Pin|Id|说明|备注|Reason|Analysis)\s*[:：]/i.test(text) ? 'field' : null;
};
...
function parseLegacyInner(content) {
    ...
    for (const source of stripRecordWrappers(content, lineAnchorKind, completeLineStructure).split(/\r?\n/)) { const text = cleanLabel(source); if (!text) continue;
        ...
        else if (current && /^Desc\s*[:：]/i.test(text)) current.desc = fieldValue(text, 'Desc'); else if (current && /^Next\s*[:：]/i.test(text)) current.next = fieldValue(text, 'Next'); else if (current && /^Cue\s*[:：]/i.test(text)) current.cue = fieldValue(text, 'Cue'); else if (current && /^Adult\s*[:：]/i.test(text)) current.adult = bool(fieldValue(text, 'Adult')); else if (current && /^Dormant\s*[:：]/i.test(text)) current.dormant = bool(fieldValue(text, 'Dormant')); else if (current && /^Id\s*[:：]/i.test(text)) current.id = fieldValue(text, 'Id');
    }
...
export function serializeLines(model, { includeCue = true, includeAdult = true, includeDormant = true, includeId = true, assignIds = includeId } = {}) {
    const seen = new Set();
    const blocks = (Array.isArray(model) ? model : []).map(item => {
        ...
        return [row, l.desc ? `Desc: ${l.desc}` : '', l.next ? `Next: ${l.next}` : '', includeId && l.id ? `Id: ${l.id}` : '', includeCue && l.cue ? `Cue: ${l.cue}` : '', includeAdult && l.adult ? 'Adult: true' : '', includeDormant && l.dormant ? 'Dormant: true' : ''].filter(Boolean).join('\n');
    });
...
function tolerantBlocks(inner) {
    ...
    for (const raw of stripRecordWrappers(inner, lineAnchorKind, completeLineStructure).split(/\r?\n/)) {
        ...
        if (/^(?:Cue|Pin|Id|Dormant|说明|备注|Reason|Analysis)\s*[:：]/i.test(text)) { block.lastText = null; continue; }
