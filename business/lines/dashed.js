...
export function dashedTargetCount(n, { min = 2 } = {}) {
    const floorMin = Math.max(1, Math.floor(Number(min) || 1));
    return Math.max(floorMin, Math.floor(Number(n) || 0));
}
...
export function resolveDashedTopics(options = {}, { theme = '', random = Math.random } = {}) {
    if (Array.isArray(options.topics) && options.topics.length) return options.topics.map(String).filter(Boolean);
    const pinned = String(theme || options.theme || '').trim();
    if (pinned && pinned !== 'random') return [pinned];
    if (options.nearText) return [];
    return pickRandomDashedTopics(DASHED_TOPIC_CONFIG, random);
}

export function getDashedAutoInterval(settings = {}) {
    const n = Math.floor(Number(settings.dashedAutoInterval));
...
export function buildDashedPrompt(userName, charName, avoidItems = [], options = {}) {
    ...
    let focus = broad;
    if (options.nearText) {
        focus = `优先写最近正文里刚带过、但没讲清的风物、规矩、来历或边角设定。不要随机百科，不要另起一套世界观。${userName} 和 ${charName} 只在正文已经碰到时顺带交代。`;
        const excerpt = String(options.latestStory || '').trim();
        if (excerpt) focus += `\n【最近正文里可挖的边角】\n${excerpt}`;
    } else if (topics.length === 1) focus = `本次只围绕「${topics[0]}」取材，写出若干条角度不同、互不重复的冷知识，数量随有效素材浮动。`;
    else if (topics.length > 1) focus = `本次依次围绕以下主题取材，每个主题至少考虑一条，数量随有效素材浮动，不要凑数：\n${topics.map((t, i) => `${i + 1}. ${t}`).join('\n')}`;
...
export function createDashedModule(env = {}) {
    ...
    async function run(options = {}) {
        ...
        try { const ctx = env.context(), user = ctx.name1 || '用户', char = ctx.name2 || '角色', cfg = env.loadConfig(); if (!cfg.url || !cfg.key) throw makeDiagnosticError('config-missing'); const topics = selected.map(value => { if (value === 'custom') return String(options.customValue || '').trim(); const item = DASHED_TOPIC_CONFIG.find(x => x.value === value); return item ? item.prompt(value === 'user' ? user : value === 'char' ? char : '') : ''; }).filter(Boolean); const prompt = [buildDashedPrompt(user, char, reroll ? [] : parse(DASHED_AVOID_COUNT), { topics, count, nearText: options.nearText === true || (!manual && !topics.length), latestStory: options.latestStory }), options.promptAddon].filter(Boolean).join('\n\n'); const raw = await env.callApi(ctx, prompt, cfg, user, char, ctrl.signal, 0, { ...(reroll ? { reroll: true, module: 'dashed' } : {}), promptMode: 'creative', diagnosticModule: 'dashed', diagnosticSink: diagnostic.sink }); if (controller !== ctrl) return; if (env.chatId() !== chatId) { busy = false; controller = null; return; } const returned = dashedItemsFromRaw(raw); if (!returned.length) throw diagnostic.rejected(makeDiagnosticError('parse', { phase: 'parse' }), { phase: 'parse', reasonCode: 'dashed-no-items' }); const current = env.filterRerollItems ? env.filterRerollItems(read(), reroll) : read(); const timestamp = now(); const merged = mergeDashedItems(returned, current, timestamp, (ts, i) => `dashed-${uuid() || `${ts.toString(36)}-${i}-${random().toString(36).slice(2, 9)}`}`); diagnostic.accepted({ phase: 'validation', reasonCode: merged.added.length ? 'dashed-valid' : 'dashed-duplicate' }); let committed; try { committed = merged.added.length ? await commitConfirmed(merged.items, timestamp, () => controller === ctrl && env.chatId() === chatId) : { items: merged.items, removed: [], ok: true }; } catch (cause) { const status = Number(cause?.saveResult?.status ?? cause?.status); const error = makeDiagnosticError('save', { phase: 'save', ...(Number.isInteger(status) ? { status } : {}) }); if (cause?.saveResult) error.saveResult = cause.saveResult; throw diagnostic.rejected(error, { phase: 'save', reasonCode: 'dashed-save-failed' }); } if (committed.ok !== true) { const status = Number(committed.saveResult?.status); const error = makeDiagnosticError('save', { phase: 'save', ...(Number.isInteger(status) ? { status } : {}) }); if (committed.saveResult) error.saveResult = committed.saveResult; throw diagnostic.rejected(error, { phase: 'save', reasonCode: 'dashed-save-rejected' }); } diagnostic.committed({ reasonCode: committed.stale ? 'dashed-saved-stale' : merged.added.length ? 'dashed-saved' : 'dashed-no-change' }); if (committed.stale || controller !== ctrl || env.chatId() !== chatId) { busy = false; controller = null; return { status: 'cancelled', reason: 'committed-but-stale', committed: true }; } const kept = new Set(committed.items.map(item => item.id)); const addedCount = merged.added.filter(item => kept.has(item.id)).length; busy = false; controller = null; try { if (manual && env.getSettings().notifyMode !== 'off') { const suffix = addedCount < count ? `（实际有效新增 ${addedCount} 条）` : ''; env.toast(addedCount ? `已新增 ${addedCount} 条冷知识${suffix}` : '本次内容与已有冷知识重复，没有新增'); } if (addedCount && options.auto !== true) env.onActivity?.({ source: 'dashed', items: merged.added.filter(item => kept.has(item.id)).map(item => ({ module: 'dashed', title: String(item.text || '').slice(0, 40), action: 'add', ref: item.id })), snapshot: { dashed: current }, after: { dashed: committed.items } }); refresh(); } catch (error) { diagnostic.uiFailed(error, { reasonCode: 'dashed-ui-refresh-failed' }); } return { status: 'updated', added: addedCount, items: merged.added.filter(item => kept.has(item.id)) }; } catch (err) { if (controller !== ctrl) return; busy = false; controller = null; if (err?.name === 'AbortError' || env.chatId() !== chatId) return; env.logDiagnostic?.(safeDiagnosticLog('dashed', err?.phase || 'request', err, { background: !manual })); panelError = `生成失败：${diagnosticMessage(err)}`; refresh(); if (manual || env.getSettings().notifyMode === 'full') env.toast(`冷知识生成失败：${diagnosticMessage(err)}`, true); }
    }
    ...
    async function onAiFloor(messageId, { blocked = false, owed = false, latestStory = '' } = {}) {
        ...
        if (blocked) {
            ...
            autoFloor = mid;
            return { status: 'skipped', reason: 'stagger' };
        }
        if (owed) {
            autoFloor = mid;
            return run({ auto: true, nearText: true, latestStory });
        }
        const tick = tickFloorGate({ lastFloor: autoFloor, counter: autoCount, messageId: mid, interval, blocked: false });
