...
export function createDashedModule(env = {}) {
    ...
    const commit = (items, ts = now()) => { const result = pruneDashedItems(items, keep(), env.getSettings().dashedCleanupEnabled !== false); if (result.items.length) return { ...result, ok: env.writeStore(key(), { items: result.items.map(item => ({ ...item, sourceKey: item?.sourceKey || dashedSourceKey(item.text) })), ts }) === true }; env.removeStore(key()); return { ...result, ok: true }; };
