...
function vectorPromptContext(vectorContext = {}) {
    ...
    return `\n【机器数据：本轮真实唯一 Ticket 与 6×3 Cue】\n每张票的三项影响角度作用于该线实际选择的主动方与事件，只是影响角度，不是确定结果；不得自行换票、改标签或造票。${poolContract}\n${fresh || '（无）'}\n新建非终态线可从以上 ${freshTickets.length} 张票中选择任意不重复子集并调整输出顺序；每条新线恰好使用一张真实 Ticket，不适合的票可以不用。Ticket 不得缺失、重复、改写或伪造；旧线不得使用 Ticket。${rerollNote}${rerollAvoidance}\n【本地锁线只读背景】\n${pinnedBackground}\n锁线已由本地完整保留，不输出、不改写、不终结、不分票。\n【旧线既有 Cue】\n${retained}\n【旧线无 Cue】\n${legacy}`;
}
export function buildLinesPrompt(userName = '用户', charName = '角色', perspective = 'user', previousRaw = '', scale = 'auto', vectorContext = {}, adultMode = 'off', lineDirection = 'natural') {
    ...
    const countContract = seedRun
        ...
        : '自然推进必须逐条原名、完整返回每条旧未锁活线（包括本轮刚进入终态者）。本轮每有一条旧未锁活线进入收束或淡出，必须新建至少一条非终态新线顶上，有正文依据，不得为凑数编造陌生人物或阴谋；没有线进入终态时不要为了补位硬建，但仍可按证据自由新建。提交后的未锁非终态自动线不得超过 8 条。旧线进入终态会腾出容量；锁线终态不占自动池、不强制补；除此不设主动方或单轮出生配额。';
