...
const SIDE_OFF = Object.freeze({ outline: false, lines: false, space: false, theater: false, almanac: false, slip: false, law: false });
...
export function openSideView(h, view) {
    ...
    if (view === 'slip') {
        if (h.slipOn?.()) return { status: 'same' };
        h.setModes?.(SIDE_OFF);
        h.enterSlip?.();
        return { status: 'slip' };
    }
    if (view === 'law') {
        if (h.lawOn?.()) return { status: 'same' };
        h.setModes?.(SIDE_OFF);
        h.enterLaw?.();
        return { status: 'law' };
    }
