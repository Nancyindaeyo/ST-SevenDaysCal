...
export function renderAxisToolbar(actionMenuHtml, history = {}) {
    const onLedger = axisState._almanacSheet === 'ledger';
    const axisHistory = historyButtonHtml({
        disabled: history.axis?.historyDisabled,
        title: history.axis?.historyTitle,
        extraClass: 'sp-icon-btn',
        module: 'axis',
    });
    const ledgerHistory = historyButtonHtml({
        disabled: history.ledger?.historyDisabled,
        title: history.ledger?.historyTitle,
        extraClass: 'sp-icon-btn',
        module: 'ledger',
    });
    return `<div class="sp-alm-toolbar">
        ...
        ${onLedger ? `<div class="sp-alm-tools">${ledgerHistory}</div>` : `<div class="sp-alm-tools">
            ...
            <div class="sp-alm-wide-tools">
                ${axisHistory}
                <button class="sp-icon-btn sp-alm-gen" title="生成节日（AI 按世界观逐月考虑，按素材生成）" aria-label="生成节日"><i class="fa-solid fa-wand-magic-sparkles"></i></button>
            ...
            <div class="sp-alm-narrow-tools">${axisHistory}${actionMenuHtml('almanac')}</div>
        </div>`}
    </div>`;
}
