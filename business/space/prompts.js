...
· 【点】从故事“今天”开始安排我／TA 接下来 3 天的事项，并另列“未来”，不是人物此刻状态卡。换日时格子机械前移（未锁的昨天记完成，锁定滚进未来），不调 API；时旅仍按目标日重排。也不潜伏注入。更新用面板顶上的刷新账本：勾选模块后「按正文对齐」或「重新生成」。空账时用「生成账本」按 面→点→线→轴→刻度标注→冷知识 排队。设置→推进设置→日期与对齐里可打开「点/线按正文自动对齐」。头部图钉「固定 TA」只把 TA 留在 TA▾ 抽屉，不是锁事项。每条的 ⋮ 菜单含编辑／锁定／注入／删除；点的锁定只保证同名事项刷新时不被删除，时间和说明仍可能随新剧情推进。
...
· 【棱】生成不进入正式楼层的纯文字番外。可空输入；挂载世界书抽签，一次 1～3 条，篇幅跟模板走，插件不再限字数。当前篇可点「续写」（可选写接下来想看什么）。不再做 HTML 美化。喜欢的条目导出为新世界书后再自行导入兔子镜。手写模板库仍在设置→数据设置→小剧场模板库。
