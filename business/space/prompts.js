...
function widgetContract(kind, calDescText) {
    ...
    if (kind === 'line_widget') return [
        ...
        `<line_widget>`,
        `Line: name|stage|when|agency|stall|pin`,
        `Desc: 事件线当前状态、背景与有关各方位置（30 字左右）`,
        `Next: 紧邻下一步或恢复条件（20 字左右）`,
        `</line_widget>`,
        `- stage 只能是 起线、延展、成形、收束、淡出；五阶段共用同一生命周期。`,
        `- 每轮依据连续正文证据与人物动机，自由判断下一变化应当激化、维持、缓和、转向、解决或淡出；分歧、关系张力、彼此试探或立场摩擦不等于必须扩大伤害。`,
        `- 阶段只描述生命周期位置，不构成升级命令。成形表示影响变得明确而非事态极端化；收束可表示解决、和解、形成新平衡或事务落定；淡出表示不再值得持续追踪。没有连续正文证据与充分动机时，不得突然扩大伤害或制造不可逆后果。`,
...
function outputModeBlock(intent = {}, calDescText = '') {
    ...
    if (intent.action === 'semantic-route') {
        ...
        return [
            ...
            `- 卡片只是建议，作者不会在这里一键写账；若用户要落地改账，请另给一份改账意图（跑法 / 要动 / 条目 / 不要动 / 依据）。`,
...
· 【面板与设置入口】点屏幕上的「构画」悬浮球打开主面板；左侧页签依次是日台、对账灯、点、轴、线、面、间、棱、坐标。切到模块后，主面板标题旁的圆圈问号显示该模块教程。悬浮球的显示开关是标题栏右上角的**空心圆点**图标（悬停名“悬浮按钮”），**它不在设置里**；藏起后可从酒馆扩展／魔杖菜单再次打开构画。齿轮进入设置：最上方是「启用构画」「时间戳」，下面四个折叠分组是通用设置／注入与内容设置／推进设置／数据设置。
...
· 【时间戳与故事星期】时间戳是全局时间锚点，包含星期判定。设置最上方的「时间戳」默认开，由主楼 AI 根据提示词随回复输出，构画只负责读取、解析和展示，不会自行生成；是否出现、格式是否完整及时间是否合理，取决于主楼模型是否遵循提示词和主楼剧情质量，主楼未输出或格式不完整时构画无法凭空补出可靠时间，可能导致时间判断失真。完整提示词编辑在设置→通用设置→提示词与标签；当前 AI 楼的开始／结束时间戳完整时，它优先决定今天与星期。缺失或不完整时，若正文或柏宝书写明了公历年月日，构画会按那个日期算出星期；没有年月日、或用的是自定义历法，才显示“星期未记录”，不会拿点日程里的假年硬猜。用户亲自设置的「校准故事时间」是兜底锚；后续完整时间戳会自动接管，「恢复自动」会立即重新判断当前楼。
...
· 【对账灯】两个子页：冲突、搜索。冲突只比对账本，不读正文。可对照最新楼、先看再写，或追从上次对齐到现在。勾选进待改篮，可手改、拿到间、或按打架一起改。刷新账本也在灯上。一天多个地点不是打架。柏宝书只对照。永不注入。
· 【棱】生成不进入正式楼层的纯文字番外。可空输入；挂载世界书抽签，一次 1～3 条，篇幅跟模板走，插件不再限字数。当前篇可点「续写」（可选写接下来想看什么）。不再做 HTML 美化。喜欢的条目导出只收展现形式，并加进手写模板堆「构画-棱-小剧场模板」。手写模板库仍在设置→数据设置→小剧场模板库。
· 【坐标】是 AI 楼层正文收藏副本，不是完整样式快照。入口开关在设置→通用设置→显示与通知管理→「收藏此楼入口」；只出现在 AI 楼。点楼层上的坐标形图标收藏，收藏后选择标签；再点同一图标取消。坐标页可按角色、标签或棱浏览，棱里每一场小剧场单独成一组；可筛选／管理标签和全屏查看。删除收藏只删副本，不影响原楼。
