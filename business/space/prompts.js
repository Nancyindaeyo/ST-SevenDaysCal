...
· 【点】从故事“今天”开始安排我／TA 接下来 3 天的事项，并另列“未来”，不是人物此刻状态卡。默认不随日期自动重排，也不潜伏注入。更新用面板顶上的刷新条：勾选模块后「按正文对齐」或「重新生成」。设置→推进设置→日期与对齐里可打开「点/线按正文自动对齐」。头部图钉「固定 TA」只把 TA 留在 TA▾ 抽屉，不是锁事项。每条的 ⋮ 菜单含编辑／锁定／注入／删除；点的锁定只保证同名事项刷新时不被删除，时间和说明仍可能随新剧情推进。
