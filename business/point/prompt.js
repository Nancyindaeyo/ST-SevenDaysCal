...
【日期说明】
Day 1 从剧情当前时间节点开始。若正文已写明年月日（如 2024年8月31日），就以那一天为今天，向后推演；不要另起一套日期，也不要回填已经发生过的时间。
${pinnedBlock}
【理想输出结构】
...
【Future 说明】
Future 收录 Day 3 之后的事项。time 字段必须写具体日期（如 5月4日 上午），不要只写「未定」「以后」；只有正文完全给不出月日时才允许「未定」。不得重复 Day 1-3 已安排的同一事件；允许基于剧情走向合理推测，但不能凭空捏造从未提及的约定或承诺。`;
}
